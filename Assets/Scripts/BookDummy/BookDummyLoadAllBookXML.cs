...
public class BookDummyLoadAllBookXML : MonoBehaviour {
    [Tooltip("默认本地文件XML文件")]
    ...
    private void Start()
    {
        ...
        if (loadAssetIsResources)
        {
            ...
            if (!File.Exists(path))
                GenerateXML.GetBookContentByFile(bookName, () => BookDummy.Instance.LoadBook(path));
            else
                BookDummy.Instance.LoadBook(path);
        }
