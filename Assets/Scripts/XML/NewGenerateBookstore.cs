...
namespace PJW.Book
{
    public class NewGenerateBookstore : MonoBehaviour
    {
        ...
        [HideInInspector]
        public bool isCanClickBook = true;
                ...
                if (Input.GetMouseButtonDown(0))
                {
                    if (Physics.Raycast(myray, out myhit, 1000000))
                    {
                        ...
                        if (myhit.transform.name == "Box03")
                        {
                            ...
                            PlayerPrefs.SetString("AssetBundle", "DinosaurChangeFoam2_L.DinosaurChangeFoam2_L");
        ...
        public IEnumerator LoadAllBookByXML(string bookXML, string name, string currentClassType)
        {
        ...
        /// <param name="name">当前书籍所属类别</param>
        /// <param name="currentClassType">当前书籍所属班级类别</param>
        public IEnumerator LoadAllBookByJSON(string bookJSON, string name, string currentClassType)
        {
            if (currentShowObject.Count > 0)
            {
                foreach (var item in currentShowObject)
                    ...
                    foreach (var classType in bookType.ClassTypes)
                    {
                        ...
                        if (classTypeName.Equals(currentClassType))
                        {
                            ...
                            foreach (var book in classType.Book)
                            {
                                ...
                                WWW www = new WWW("file:///" + book.BookImage);
        ...
        public void LoadAllBookByAssetBundle(string assetBundleName, string bookType, string classType)
        {
            if (currentShowObject.Count > 0)
            {
                foreach (var item in currentShowObject)
            ...
            for (int i = 0; i < allBookNames.Length; i++)
            {
                ...
                if (allBookNames[i].Split('/')[3] == bookType.ToLower())
                {
                    str = allBookNames[i].Split('/')[3] + "_";
                    if (allBookNames[i].Split('/')[4] == classType.ToLower())
                    {
                        str += allBookNames[i].Split('/')[4];
                        Debug.Log(str + " **** this is book of type ");
                        if (allBooks.ContainsKey(str))
                        {
                            foreach (var item in allBooks[str])
                            {
                                bookNum++;
                                Debug.Log(item.name);
                                Texture texture = GameCore.Instance.asset.LoadAsset<Texture>(item.name);
                                item.transform.Find("book/Box03").GetComponent<Renderer>().material.mainTexture = texture;
                                item.SetActive(true);
                                currentShowObject.Add(item);
                            }
                            return;
                        }
                        currentPage.Add(allBookNames[i].Split('/')[5]);
                    }
                }
                //allBookNames[i] = allBookNames[i].Split('/')[5];
            }
            GameObject parent = new GameObject("AllBooks");
            allBooks[str] = new List<GameObject>();
            for (int i = 0; i < currentPage.Count; i++)
            {
                if (currentPage[i].Split('.')[1].Equals("jpg"))
                {
                    ...
                    temp.name = currentPage[i].Split('.')[0];
                    //temp.transform.Find("New Text").GetComponent<TextMesh>().text = temp.name;
                    temp.transform.parent = parent.transform;
                    Texture texture = GameCore.Instance.asset.LoadAsset<Texture>(currentPage[i]);
                    temp.transform.Find("book/Box03").GetComponent<Renderer>().material.mainTexture = texture;
                    bookNum++;
                    allBooks[str].Add(temp);
                    currentShowObject.Add(temp);
